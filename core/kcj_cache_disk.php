...
final class kcj_Cache_Disk {
	...
	*/
	...
	public static function check_expiry() {
		...
		// kcj cache options
		$options = kcj_Cache::$options;

		// check if expires is active
		if ($options['dch_expires'] == 0) {
			return false;
		}

		$now = time();
		$expires_seconds = 3600*$options['dch_expires'];
	...
	*/
	...
	public static function kcj_cache_size($dir = '.') {
		...
		foreach ( $objects as $object ) {
			...
			// check if dir
			if ( is_dir($object) ) {
				$size += self::kcj_cache_size($object);
			} else {
